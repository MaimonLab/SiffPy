...
from ...siffplot.roi_protocols.rois import ROI
...
class PhasePlotter(SiffPlotter):
    ...
    def estimate_phase(self, roi : ROI = None, vector_timeseries : np.ndarray = None, **kwargs)->np.ndarray:
        """
        ...
        vector_timeseries : np.ndarray (optional)
            ...
            numpy array of fluorescence data.
    ...
    def wrapped_error_plot(x_axis : np.ndarray, central_data : np.ndarray, error : np.ndarray,
        ...
        if len(error.shape) > 2:
            ...
            upper_error = error.flatten()
            lower_error = error.flatten()
        else:
        ...
        hv.Spread((x_axis,overflow_array,overflow_err), **kwargs).opts(line_color=None) * \
        hv.Spread((x_axis,underflow_array,underflow_err), **kwargs).opts(line_color=None)
