...
-    Unify `SiffTrac` and `SiffPlot`
-    Deprecate `siffutils` in favor of `siffpy.core`
...
(though all rely on the `core` submodule, and that in turn relies on the `siffutils` submodule),
...
functionality related to image processing, such as image registration. TO BE DEPRECATED!!! WILL USE CORE
