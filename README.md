...
### TODOS:
-    Migrate to `siff-napari` plugin based interface, instead of the 
`NapariInterface` section of `siffplot`.
-    Separate `ROI` tools from `plot` tools. There's no reason they
should be tied together once I've moved out of `HoloViews`.
