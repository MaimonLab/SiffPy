...
DEBUG(
const FrameData getTagDataDebug(const uint64_t IFD, const SiffParams& params, std::ifstream& siff, std::ofstream& logstream){
    // return a FrameData structure that has parsed all the tag information at location:
    // IFD
    ...
    logstream << "Reading tag data from IFD " << IFD << std::endl;
    ...
    logstream << "Reading " << numTags << " tags." << std::endl;

    // tag parsing TODO: SHOULD BE INLINED SOMEWHERE ELSE
    for(uint64_t tagNum = 0; tagNum < numTags; tagNum++) {
        siff.read(thisTag, params.bytesPerTag);

        logstream << "Reading tag " << tagNum << std::endl;
        ...
        logstream << "Tag ID: " << tagID << std::endl;
        ...
        logstream << "Content value: " << contentVals << std::endl;
    ...
    logstream << "Done reading tags." << std::endl;
    
    if (frameData.dataStripAddress<frameData.endOfIFD) throw std::runtime_error("Invalid data strip address read.");    
    siff.clear();

    logstream << "Returning frame data." << std::endl;
    return frameData;
}
)
